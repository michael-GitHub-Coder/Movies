           ...
           <div className="flex grid grid-cols-4 gap-6">
            { 
                    movies && movies.map(data => {
                        ...
                        <Link key={data.id} onClick={(e) => {e.preventDefault(); viewMovies(data.id)}} ><img src={data.image}  className="w-44 h-64 max-w-xs overflow-hidden shadow-lg hover:transform hover:translate-x-4 hover:translate-y-2 transition-transform duration-300"/></Link>
                        )
                    })
                }
           </div>
